...
function AtakYap(min,max) {
  ...
  canavar.value.can = canavar.value.can - RandomNumber(min, max);
  CanavarAtak();
  turSayisi++;
...
function CanavarAtak(){
  oyuncu.value.can = oyuncu.value.can - RandomNumber(6, 10);
}

function PotBas(){
 oyuncu.value.can = oyuncu.value.can + RandomNumber(5, 
 10)
